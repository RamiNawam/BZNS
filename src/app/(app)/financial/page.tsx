    ...
    <div className="card overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          <div className={`inline-flex items-center justify-center h-9 w-9 rounded-xl ${iconBg}`}>
  ...
  return (
    <div className="flex items-center gap-6">
      <svg width={160} height={160} viewBox="0 0 160 160" className="shrink-0">
        ...
        {/* Center text */}
        <text x={CX} y={CY - 6} textAnchor="middle" className="fill-slate-900 font-heading" fontSize="18" fontWeight="700">
          {fmt(total)}
        </text>
      ...
      <div className="space-y-1.5 min-w-0 flex-1">
        {categories.map((cat, i) => (
          <div key={cat.key} className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-600 truncate flex-1">{cat.label}</span>
            <span className="text-xs font-bold text-slate-700 tabular-nums">{fmt(cat.amount)}</span>
          </div>
        ))}
      </div>
    </div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" role="img" aria-label="12-month projection">
        {yTicks.map(({ v, y }) => (
          <g key={v}>
            ...
            <text x={PL - 4} y={y + 4} textAnchor="end" fill="#94a3b8" fontSize="9">{v >= 1000 ? `$${(v / 1000).toFixed(0)}K` : `$${v.toFixed(0)}`}</text>
          </g>
        ))}
        {gstY > PT && gstY < PT + iH && (
          <>
...
function BreakEvenCard({
  ...
  const progressPct = result.breakEvenMonthlyRevenue > 0
  ...
  return (
    <div className="card p-5 space-y-3">
      ...
      <div className="relative h-3 rounded-full bg-slate-100 overflow-hidden">
        ...
        />
        {/* Break-even marker */}
        <div className="absolute inset-y-0 w-0.5 bg-slate-900" style={{ left: '100%', transform: 'translateX(-1px)' }} />
      </div>
    ...
    <div className="p-5 space-y-4">
      ...
      <div className="space-y-2">
        {summary.deductions.map((d) => (
          <div key={d.key} className="flex items-start justify-between py-2 border-b border-slate-50 last:border-0">
            <div className="min-w-0">
...
function FundingImpactCard({
  ...
  const impact = useMemo(
    () => calculateFundingImpact(estimatedFunding, monthlyExpenses, monthlyTakeHome),
...
export default function FinancialPage() {
  ...
  const updateExpense = (key: string, newAmount: number) => {
    setExpenseCategories((prev) =>
      prev.map((c) => (c.key === key ? { ...c, amount: Math.max(0, newAmount) } : c)),
    );
  };
